...
function SearchInput({wFull}:Props) {
  return (
    <form className={`relative flex gap-4 overflow-hidden  ${wFull ? 'w-full' : 'md:w-[580px]'}`}>
        ...
        <input type="text" className={`pl-10 pr-2 py-2 bg-bg-2 text-gray-200 font-medium border-[2px] border-[rgba-2] rounded-lg outline-none transition-all duration-300 ease-in-out focus:w-full focus:border-rgba-0 ${wFull ? 'w-full' : 'w-[90%]'}`}/>
